...
depends on the ``deployment_mode`` in ``scaldys-project.toml`` and the build command
being run.
...
.. list-table::
   ...
   * - 2
     - The source root directory (``src/`` by default, or the value of
       ``[cython] source_root`` in ``scaldys-project.toml``) must exist.
     - All build commands
   ...
   * - 5
     - The Windows packaging directory (``packaging/windows/`` by default,
       or the value of ``[windows] script_dir`` in ``scaldys-project.toml``) must
       exist.
